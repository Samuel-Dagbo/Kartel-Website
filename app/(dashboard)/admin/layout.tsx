...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-kartel-black' : 'bg-kartel-cream'}`}>
      <Sidebar
        isOpen={sidebarOpen}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative hidden sm:block">
              ...
              <input
                type="text"
                placeholder="Search..."
                className={`pl-9 pr-4 py-2 rounded-xl text-sm w-64 transition-colors focus:outline-none focus:border-kartel-gold/30 focus:ring-1 focus:ring-kartel-gold/10 ${
                  isDark
                    ? 'bg-white/[0.03] border border-white/[0.06] text-white placeholder:text-white/20'
                    : 'bg-black/[0.03] border border-black/[0.08] text-kartel-black-900 placeholder:text-black/25'
                }`}
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={`flex items-center gap-2 ml-2 pl-2 border-l transition-colors ${
              ...
              <div className="hidden sm:block">
                ...
                <p className={`text-[10px] ${isDark ? 'text-white/30' : 'text-kartel-black-400'}`}>
                  Administrator
                </p>
              </div>
            </div>
