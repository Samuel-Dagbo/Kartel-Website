  ...
  return (
    <div className="min-h-screen flex bg-kartel-black-950">
      ...
      <aside className="w-72 border-r border-white/[0.06] bg-gradient-to-b from-kartel-black-900 to-kartel-black-950 flex flex-col sticky top-0 h-screen">
        ...
        <div className="p-6 border-b border-white/[0.06]">
          <Link href="/" className="block">
            ...
            <span className={`text-[10px] block tracking-[0.3em] mt-1 font-medium ${isAdminRoute ? 'text-kartel-gold/60' : 'text-white/40'}`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          <div className="text-[9px] font-bold text-white/25 uppercase tracking-[0.2em] px-4 mb-3">
            {isAdminRoute ? 'Main Menu' : 'Menu'}
          </div>
          {navItems.map((item) => {
            const isActive = pathname === item.href || (item.href !== '/admin' && item.href !== '/customer' && pathname?.startsWith(item.href))
            return (
              <Link
                key={item.href}
                href={item.href}
        ...
        <div className="p-4 border-t border-white/[0.06] space-y-2">
          ...
          >
            ...
            <span className="text-sm">{isAdminRoute ? 'View Website' : 'Continue Shopping'}</span>
          </Link>
          <button
      ...
      <main className="flex-1 overflow-y-auto bg-kartel-black-950">
        <div className="p-8">
          <div className={isAdminRoute ? "max-w-7xl mx-auto" : "max-w-5xl mx-auto"}>
            {children}
          </div>
        </div>
      </main>
